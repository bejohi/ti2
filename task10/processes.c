...
static int background_process(void)
{
	while (1) {
		so_io();
	}
...
int init(void)
{
	...
	puts("[INIT] Starting a bunch of background processes");
	for (int i = 0; i < 42; i++) {
		char namebuf[32];
		snprintf(namebuf, sizeof(namebuf) - 1, "bg-%d", i);
		if (so_create(namebuf, background_process, SO_PRIO_NORMAL) == -1) {
