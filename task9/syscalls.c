...
so_pid_t so_create(const char *name, int (*program)(void))
{
	pthread_mutex_lock(&core_mutex);
	...
	sched_enqueue(&sched_ready_q, pcb);
	pthread_mutex_unlock(&core_mutex);
...
so_pid_t so_wait(int *code)
{
...
#endif
	if (!sched_ready_q.head && !sched_blocked_q.head && !sched_io_q.head) {
	...
	block();
	sched_enqueue(&sched_blocked_q, sched_me);
	pthread_mutex_unlock(&core_mutex);
	...
	sched_enqueue(&sched_ready_q, sched_me);
...
#endif
	block();
	sched_enqueue(&sched_io_q, sched_me);
